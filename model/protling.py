                ...
                for _ in range(configs[label].queueing['num_evaluation_invocations']):
                    ...
                    for idx, dict_ in dicts.items():
                        if 'tertiary'  in dict_:
                            np.savetxt(os.path.join(outputs_dir, str(idx,'utf-8') + '.tertiary'), dict_['tertiary'], header='\n')
                        if 'recurrent_states' in dict_:
                            np.savetxt(os.path.join(outputs_dir, str(idx,'utf-8') + '.recurrent_states'), dict_['recurrent_states'])
